...
npm run verify:pwa          # PWA 层验收（34 项，跑生产构建产物 + 自建静态服务器）
...
13. 移动端（**必须真机**，桌面浏览器验不了）：用 `npm run build` 的产物部署到 HTTPS 地址后，在 iOS Safari 里点分享 → 添加到主屏幕，再从主屏幕图标打开——应没有浏览器地址栏、`display-mode: standalone` 为真，且「存储与安装」显示「独立窗口（已安装到主屏幕）」。这一条的目的是让本地数据进入独立存储分区，避开 iOS「7 天无交互即清空可写存储」的规则；**安装不能替代备份**，重要数据仍要用上面的导出留一份。
