...
此工程包含账号认证、Asia/Shanghai 日期引擎、日期模式、固定周一至周日周视图、工作日规划闭环（五项准备状态、三餐、晨间事项、健身和自定义事项），以及选项管理（常用食物 / 固定补剂 / 健身项目的新增、改名、排序、启停、删除，按账号隔离）。当前开发模式使用浏览器 IndexedDB 保存账号和业务数据，不依赖 Supabase 或网络；上线前再将服务层整体迁移到云端数据库。
...
npm run check:local-data # 本地数据结构回归（15 项，无需浏览器）
npm run verify:local     # 真实浏览器闭环验收（52 项，自带临时开发服务器，无需先跑 dev）
```
...
`npm run check:local-data` 同样直接导入源码，断言 IndexedDB 迁移表「只追加」（`DB_VERSION` 与迁移表自洽、v1 四张表不被改写、仓库定义与迁移表一一对应）、三张选项表带 `user_id` 索引，以及界面层没有直接引用 `localDb` / `indexedDB` / `supabase`、没有写死任何示例选项名称。

`npm run verify:local` 会自动挑一个空闲端口拉起本项目的 Vite 开发服务器，并拉起本机 Chrome 或 Edge 的无头实例，实际走一遍
「v1 老库冷升级 → 登录页 → 注册 → 周视图 → 历史日只读 → 模式切换与恢复默认 → 切日期后保存状态归零 → 未来空日期引导 → 三餐保存 → 准备项进度 → 健身面板 → 自定义事项增改删 → 复制昨天不带准备勾选且不改目标日模式 → 选项页增改排序启停删与补剂时段分组 → 刷新恢复 → 账号间选项隔离 → 桌面/手机视口无溢出 → 退出登录」共 52 项检查，
并检查控制台无 error / warning。结束时自动关闭浏览器与临时服务器，不残留进程、不占用固定端口。
其中冷升级用例会用浏览器调试协议把入口模块的响应换成空模块，让页面停在「同源但没跑应用」的状态，先造出一个真正的 v1 老库，再放开加载、由应用自己把它升到 v2，用来验证迁移只追加且老数据不丢。
...
4. 使用两个本地账号验证各自的三餐、自定义事项和选项（食物 / 补剂 / 健身）相互不可见；尝试新增、修改、删除历史从属记录必须被服务层拒绝。
