...
### 8.4 添加事项改为居中卡片（2026-09-16 第三轮）

**需求**：「把添加事项的下方抽屉样式换成中央卡片」。

**做法**：没有另起一个组件，而是把 `BottomSheet` 泛化成 `EditorDialog`，新增
`placement: 'bottom' | 'center'`：

- 自定义事项（添加 / 编辑）用 `center`——只有三个短字段，居中更聚焦，
  视线不必甩到屏幕最下方；
- 三餐 / 补剂 / 健身 / 晨间 / 选项编辑仍用 `bottom`——长表单在抽屉里更舒展，
  且单手够得到下方操作区。

两种形态共用同一套结构（标题固定 / 中间 `.editor-body` 独立滚动 / 操作区钉底）与
body 滚动锁定，因此第 8.2 条那套软键盘规则对两者同时生效。相关改动：
`.sheet-body` 更名 `.editor-body`（两形态共用）、新增 `.center-card`、
`.modal-backdrop` 补 `padding-bottom: var(--vv-keyboard-inset, 0px)`。
两种形态的根节点都带 `data-editor-card`，验收脚本据此定位，**形态再换也不必改选择器**。

**验收**：`verify:local` **95/95**（新增一条「自定义事项编辑用居中卡片（不是底部抽屉）」，
断言 `center-card=true` 且 `bottom-sheet=false`）；`verify:pwa` **42/42**——
软键盘断言拆成两组分别驱动两种形态：底部抽屉 `16→508`、居中卡片 `67→441`，
两者的操作区都在可视区内（484 / 421 ≤ 508）。

**未动的**：选项编辑（添加食物 / 补剂 / 项目、重命名）与晨间事项仍是底部抽屉。
它们同样是短表单，要一并换成居中卡片的话是一行改动（`placement` 的取值），等确认再说。
