...
| `npm run verify:local` | **104/104** | 真实浏览器闭环，约 2.5 分钟 |
| `npm run verify:pwa` | **42/42** | PWA 层，跑生产产物 + 自建静态服务器，约 2 分钟 |
| 本地构建体积 | 91 modules / 281.47 kB（gzip 87.71 kB） | 含 PWA 外壳与移动端交互修复 |
...
验证要求：
- 三件套 + 五个 check 脚本 + verify:local 104/104（必须复现，不能只跑新的那个）。
...
验证要求：
- typecheck / lint / build 通过；check:dates 48/48、check:local-data 27/27、check:backup 55/55、check:cloud-parity 50/50、verify:local 104/104 全部复现（不能因为改的是云端就跳过本地基线）。
