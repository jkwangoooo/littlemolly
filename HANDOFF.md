...
1. ~~**移动端持久化（PWA 层）**~~ —— **已完成（2026-09-16）**，报告见 `docs/16-pwa-shell-completion-report.md`。
   - 现状：manifest / 四张 PNG 图标 / 手写 service worker / 存储状态卡片均已落地；dev 仍不注册 SW，`verify:local` 94/94 未回退，新增 `verify:pwa` 40/40。
...
- `npm run verify:pwa`：**40/40 通过**（新建）。
...
- **待办 A（PWA 层）完成**，行为基线未回退，验收基线为「三件套 + 四个 check 脚本 + `verify:local` 94 项 + `verify:pwa` 40 项」。
