...
1. ~~**移动端持久化（PWA 层）**~~ —— **已完成（2026-09-16）**，报告见 `docs/16-pwa-shell-completion-report.md`。
   - 现状：manifest / 四张 PNG 图标 / 手写 service worker / 存储状态卡片均已落地；dev 仍不注册 SW，`verify:local` 94/94 未回退，新增 `verify:pwa` 38/38。
...
- `npm run verify:pwa`：**38/38 通过**（新建）。
...
- **待办 A（PWA 层）完成**，行为基线未回退，验收基线为「三件套 + 四个 check 脚本 + `verify:local` 94 项 + `verify:pwa` 38 项」。
...
3. **「首页做成固定单页面不可滑动」→ 待定，需要产品取舍**。量化事实：执行今天 892px（只差约 135px 就能一屏），
   准备明天 1354px（要砍掉约 600px 才能一屏）。两条路线（固定外壳+内容区内部滚动 / 严格一屏+把细节收进抽屉）
   都以上面这条为底座，等用户决定后再做，未擅自开工。
